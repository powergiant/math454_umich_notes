...
\end{equation}
for all functions $\varphi(x)$ and $\psi(x)$ in the domain of $A$. This condition translates into the following integral equality,
\begin{equation}
\int_a^b \varphi(x) \left(-\psi''(x)\right) dx = \int_a^b \left(-\varphi''(x)\right) \psi(x) dx
\end{equation}
Applying integration by parts and using the boundary conditions $\varphi(a) = \varphi(b) = \psi(a) = \psi(b) = 0$, the boundary terms vanish, leading to
\begin{equation}
-\int_a^b \varphi(x) \psi''(x) dx = \int_a^b \varphi'(x) \psi'(x) dx = -\int_a^b \varphi''(x) \psi(x) dx
\end{equation}
This equality confirms \eqref{eq.ex_symm_op_1} and thus the fact that $A$ is symmetric on the given domain $\mathrm{Dom}(A)$.
\end{example}

\begin{example}
    \textbf{TODO: symmetric operator depends on domain}
\end{example}
