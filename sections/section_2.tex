...
\begin{theorem}[Orthogonality relations of Fourier cosine series]
...
\begin{theorem}[Orthogonality relations of Fourier sine series]
...
\begin{equation}\label{eq.proof_Parseval_complex_1}
    \begin{split}
        ...
        \\
        =& \sum_{n,m=-\infty}^{\infty}\alpha_n\bar{\alpha}_m \underbrace{\frac{1}{2 L} \int_{-L}^L e^{i (n-m) \pi x / L} d x}_{= 0\textit{ if }n\neq m}
        \\
...
\begin{theorem}[Parseval's theorem for Fourier cosine series] Let $f(x)$ defined on $0<x<L$ be a piecewise smooth function with Fourier cosine series 
    \begin{equation}\label{eq.Parseval_cosine_assumption}
        f(x)=A_0 + \sum_{n=1}^{\infty} A_n \cos \frac{n \pi x}{L}
    \end{equation}
    Then, the mean square $\frac{1}{L} \int_0^L |f(x)|^2 d x$ of $f(x)$ satisfies the following identity
    \begin{equation}\label{eq.Parseval_cosine}
        \frac{1}{L} \int_0^L |f(x)|^2 d x=A_0^2 + \frac{1}{2}\sum_{n=1}^{\infty}A_n^2 .
    \end{equation}
\end{theorem}
\begin{proof}
    This is homework.
...
\begin{theorem}[Parseval's theorem for Fourier sine series]
    Let $f(x)$ defined on $0<x<L$ be a piecewise smooth function with Fourier sine series 
