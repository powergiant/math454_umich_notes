...
\begin{equation}\label{eq.heat_kernel_gaussian}
  \int_{-\infty}^{\infty}e^{-Kt\mu^2+i(x-x')p}\,d\mu
...
\begin{equation}\label{eq.eigenvalue_full_line}
  \phi(x) = e^{i\mu x},\qquad \lambda = \mu^2, \qquad p\in\mathbb{R}.
\end{equation}
The eigenfunctions $e^{i\mu x}$ are labeled by a continuous parameter $\mu$, and the eigenvalues satisfy $\lambda = \mu^2 \ge 0$. This is called the \underline{continuous spectrum}, in contrast to the discrete spectrum that arises on bounded domains. The ``eigenfunction expansion'' becomes
